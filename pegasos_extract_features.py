...
if __name__ == "__main__":
    ...
    dataset = sys.argv[3]
    data_dir = '/home/lichongxuan/regbayes2/data/mat_data/'

    z = ndict.loadz(dir+'v.ndict.tar.gz')
    ...
    for i in xrange(layers):
        z_train = np.log(1 + np.exp(z['w'+str(i)].dot(z_train) + z['b'+str(i)]))
        
    z_test = test_x
    for i in xrange(layers):
        z_test = np.log(1 + np.exp(z['w'+str(i)].dot(z_test) + z['b'+str(i)]))
        
    # train_data = mat[0][0]
    train_data = z_train.T
    print train_data.shape
    train_label = train_y[:,0]
